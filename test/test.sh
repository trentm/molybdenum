...
rm -rf ${DATADIR}
mkdir -p ${DATADIR}
node $ROOT/app.js -c ${ROOT}/test/test.ini > tmp/hub.log 2>&1 &
while [[ -z "`curl -sS localhost:3334/api/ping 2>/dev/null | grep pong`" ]]; do
    sleep 1;
done
...
echo '{"repository": {"url": "tmp/eol.git", "name": "eol"}}' \
    | curl -sS http://localhost:3334/api/repos/eol -X POST -d @- >/dev/null
